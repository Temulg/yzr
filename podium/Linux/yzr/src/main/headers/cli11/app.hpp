...
#include "cli11/error.hpp"
#include "cli11/formatter_fwd.hpp"
#include "cli11/option.hpp"
#include "cli11/split.hpp"
#include "cli11/string_tools.hpp"
#include "cli11/type_tools.hpp"
...
class App
{
			   ...
			   std::string description = "")
	{
		...
		cli_11::callback_t fun = [&variable](cli_11::results_t res) { return detail::lexical_cast(res[0], variable); };
			   ...
			   std::string description = "")
	{
		...
		cli_11::callback_t fun = [&variable](cli_11::results_t res) {
			bool retval = true;
			variable.clear();
	...
	{
		cli_11::callback_t fun = [](cli_11::results_t) { return true; };
			 ...
			 std::string description = "")
	{
		...
		count = 0;
		cli_11::callback_t fun = [&count](cli_11::results_t res) {
	...
	/// `multi_option_policy(cli_11::MultiOptionPolicy::Throw)` is used.
			 ...
			 std::string description = "")
	{
		...
		if (opt->get_positional())
			throw IncorrectConstruction::PositionalFlag(name);
		opt->type_size(0);
		opt->multi_option_policy(cli_11::MultiOptionPolicy::TakeLast);
		return opt;
	}
				  ...
				  std::string description = "")
	{
		...
		cli_11::callback_t fun = [function](cli_11::results_t res) {
			...
			std::string description = "")
	{
		...
		std::string simple_name = cli_11::detail::split(name, ',').at(0);
		cli_11::callback_t fun = [&member, options, simple_name](cli_11::results_t res) {
			    ...
			    std::string label = "COMPLEX")
	{
		...
		std::string simple_name = cli_11::detail::split(name, ',').at(0);
		cli_11::callback_t fun = [&variable, simple_name, label](results_t res) {
			if (res[1].back() == 'i')
				res[1].pop_back();
			double x, y;
		...
		cli_11::Option *opt = add_option(name, fun, description, defaulted);
	...
	int exit(const Error &e, std::ostream &out = std::cout, std::ostream &err = std::cerr) const
	{
		...
		/// Avoid printing anything if this is a cli_11::RuntimeError
		if (dynamic_cast<const cli_11::RuntimeError *>(&e) != nullptr)
			return e.get_exit_code();

		if (dynamic_cast<const cli_11::CallForHelp *>(&e) != nullptr)
		{
			out << help();
			return e.get_exit_code();
		}

		if (dynamic_cast<const cli_11::CallForAllHelp *>(&e) != nullptr)
		{
